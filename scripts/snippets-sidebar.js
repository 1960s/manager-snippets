//SIDEBAR HTML GENERATION
  function setHTMLSidebar(){
    sidebarObj = {}
    let sidebarValues = []
    let sidebarHTML = []
    let sidebarKeys = ["folders", "lang", "tags", "author", "type",]
    //total values of group types, eg all tags
    sidebarKeys.forEach(el => sidebarValues.push(totalVal(el)))
    //parallel to keys
    sidebarKeys.forEach((el, i) => sidebarHTML.push(sidebarGroupHTML(el, sidebarValues[i])))
...
`
    document.getElementById(`sidebarBox`).innerHTML = allSnippets + sidebarHTML.join("")
  }
